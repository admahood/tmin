...
raw_frp_mean_25 <- list.files("data/gridded_mod14_2_5/FRP_mean",
                           pattern = "_N_", full.names = TRUE) %>%
  as_tibble() %>%
  mutate(year = str_extract(value,"\\d{4}"))
...
for(y in years){
  ...
  frp_mean <- filter(raw_frp_mean_25, year == y) %>%
    pull(value) %>%
    terra::rast() %>%
    mean
  terra::writeRaster(frp_mean, 
                     filename = paste0("data/annual_adjusted_counts_25/annual_night_frp_mean_MW_per_detection_",y, ".tif"),
                     overwrite = TRUE)
  
}


# Average night afd per pixel for use as a mask ================================
dir.create("data/masks")
adjusted_n_25 <- list.files("data/adjusted_counts_25/", 
                    full.names = TRUE, pattern = "_N_") %>%
  as_tibble() %>%
  mutate(year = str_extract(value,"\\d{4}"))

adjusted_n_1 <- list.files("data/adjusted_counts/", 
                            full.names = TRUE, pattern = "_N_") %>%
  as_tibble() %>%
  mutate(year = str_extract(value,"\\d{4}"))
...
an_25 <- adjusted_n_25 %>%
    ...
    terra::rast() %>%
    mean
an_25_df <- as.data.frame(an_25, xy=TRUE) %>%
  dplyr::rename(mean_afd_per_op = mean)

terra::writeRaster(an_25, 
                   filename = paste0("data/masks/an_25.tif"),
                   overwrite = TRUE)
...
an_1_df <- as.data.frame(an_1, xy=TRUE) %>%
  dplyr::rename(mean_afd_per_op = mean)

terra::writeRaster(an_1, 
                   filename = paste0("data/masks/an_1.tif"),
                   overwrite = TRUE)
...
p_nf<-ggplot(night_fraction_trends %>% filter(p<0.05) %>% 
  ...
  geom_raster(data = lck_df%>%
              ...
              aes(x=x,y=y), fill = "grey90")+
  geom_raster(aes(x=x,y=y,fill=Trend)) +
  # geom_raster(data = lck_df %>% filter(croplands == "Croplands"), 
  #             aes(x=x,y=y), fill = "darkorchid3") +
  scale_fill_manual(values = posneg_cols)+
  theme_void()+
...
night_frp_trends <- night_frp_trends %>%
  left_join(an_1_df)

p_frp <- ggplot(night_frp_trends %>% filter(p<0.05, mean_afd_per_op > 0.01) %>% 
         mutate(Trend = ifelse(beta > 0, "Positive", "Negative"))) +
  geom_sf(data = st_read("world.gpkg"), lwd=0.25, fill="white")+
  geom_raster(data = lck_df %>%
                filter(Burnable == "yes"), 
...
## annual night frp mean 2.5 degrees ===========================================
if(!file.exists("data/night_frp_trends_25.Rda")){
  night_frp <- list.files("data/annual_adjusted_counts_25", 
                          pattern = "annual_night_frp_mean_MW_per_detection_", 
                          full.names = TRUE) %>%
    terra::rast()
  ...
  night_frp_trends_25<-parallel_theilsen(night_frp_df,
                                      zero_to_na = TRUE,
                                      workers=4, 
                                      minimum_sample = 10) %>%
    left_join(an_25_df)
  
  save(night_frp_trends_25,file= "data/night_frp_trends_25.Rda")
}else{
  load("data/night_frp_trends_25.Rda")
}
p_frp_25 <- ggplot(night_frp_trends_25 %>% filter(p<0.05, mean_afd_per_op > 0.001) %>% 
                  mutate(Trend = ifelse(beta > 0, "Positive", "Negative"))) +
  geom_sf(data = st_read("world.gpkg"), lwd=0.25, fill="white")+
  geom_raster(data = lck_df %>%
                filter(Burnable == "yes"), 
...
ggsave(p4p,height = 5.3, width=12, filename = "out/EDF7_adf_trends_annual_4pan.png")

# writing out trends as csv for john
bind_rows(
  day_counts_trends %>% mutate(variable = "day_afd_per_op"),
  night_counts_trends %>% mutate(variable = "night_afd_per_op"),
  night_fraction_trends %>% mutate(variable = "fraction_afd_night"),
  night_frp_trends %>% mutate(variable = "night_mean_frp_per_detection")
) %>% summary()
  write_csv("out/edf7_trends.csv")
...
ggsave(p_3pan_yearly,filename = "out/EDF8a_global_trends_line_plots_year.png",
         height =12, width = 6)
# MONTHLY GLOBAL ===============================================================
...
ggarrange(p_afd, p_np, p_frp, nrow=3, labels = c("a", "","")) +
  ggsave(filename = "out/EDF8a_global_trends_line_plots.png",
         height =12, width = 6)
...
ggarrange(p_k_afd, p_k_np, p_k_frp, nrow=3, labels = c("b", "", "")) +
  ggsave(filename = "out/EDF8b_global_trends_by_koppen_line_plots.png",
         height =12, width =12)
...
ts_trend_day <- mblm(trend ~ timestep, df_afd_day %>% na.omit(), repeated = TRUE); summary(ts_trend_day)
...
ggsave(p_deco_3pan, filename = "out/ts_decompose.png", width = 8, height =12)
...
p_nf_single <-ggplot(night_frp_trends_25 %>% filter(p<0.05, mean_afd_per_op > 0.001)  %>% 
                       mutate(`Trend in Nighttime Fire Radiative Power` = ifelse(beta > 0,
                                                         "Positive", 
                                                         "Negative"))) +
  geom_sf(data = st_read("world.gpkg"), lwd=0.25, fill="white")+
  geom_raster(data = lck_df%>%
                filter(Burnable == "yes"), 
              aes(x=x,y=y), fill = "grey90")+
  geom_raster(aes(x=x,y=y,fill=`Trend in Nighttime Fire Radiative Power`)) +
  # geom_raster(data = lck_df %>% filter(croplands == "Croplands"), 
  #             aes(x=x,y=y), fill = "darkorchid3") +
  scale_fill_manual(values = posneg_cols)+
  theme_void()+
...
p_inset2 <- long_df %>%
  ...
  ggplot(aes(x=acq_year, y=mean_frp_per_detection, color = dn_detect))+
  geom_line(lwd=1) +
  scale_color_manual(values=daynight_cols)+
  theme_void() +
  scale_y_continuous(name = "",# = c(28.5, 49),labels = c(28.5, 49), 
...
ggsave(pp,filename="out/Fig5_percent_night_w_inset_nocroplandmask.png",bg="white", height=10, width=20)
